let arrayCarrito = [];
let arrayProductos = [
  //Creo mi array de objetos de productos
    {
    id: 1,
    nombre: "te",
    precio: 100,
...
const calcularCompra = arrayCarrito.reduce(
    (acc, producto) => acc + producto.precio,0
); // Utilizo el metodo reduce() y se lo asigno a una variable
...
12 cuotas;`)
    ...
    break;
default:
    alert(`No ingreso metodo de pago`);
    alert(medioDePago);
    break;
}
...
switch (cantidadDeCuotas) {
    case 3:
    alert(
        `En ${cantidadDeCuotas} cuotas el valor de cada cuota es de: $` +
        calcularCompra / 3
    );
    break;
    case 6:
...
function buscarProducto(producto) {
    ...
    console.log(`Encontrado con find: `, resultado);
}
...
function filtrarProducto(producto) {
    const resultadoDeFiltrar = arrayProductos.filter(
    (elemento) => elemento.nombre === producto
);
    console.log(`Encontrado con filter: `, resultadoDeFiltrar);
}

pedirDatosAlUsuarioParaFiltrar();
